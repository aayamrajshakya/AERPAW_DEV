...
int uper_open_type_put(
  ...
  ASN_DEBUG(
      "Open type put %s of length %" ASN_PRI_SSIZE " + overhead (1byte?)",
      td->name, size);
  ...
  do {
    ...
    ASN_DEBUG(
        "Prepending length %" ASN_PRI_SSIZE
...
static asn_dec_rval_t uper_open_type_get_simple(
  ...
  do {
    ...
    if (per_get_many_bits(
            pd, (buf == NULL) ? NULL : buf + bufLen, 0, chunk_bytes << 3)) {
      FREEMEM(buf);
      ASN__DECODE_STARVED;
    }
    bufLen += chunk_bytes;
...
#define UPDRESTOREPD                                                           \
  ...
  ASN_DEBUG(
      "OpenType %s pd%s old%s unclaimed=%d, repeat=%d", td->name,
  ...
  if (pd->nboff != pd->nbits) {
    ASN_DEBUG(
        "Open type %s overhead pd%s old%s", td->name, asn_bit_data_string(pd),
...
static int uper_ugot_refill(asn_per_data_t* pd) {
  ...
  ASN_DEBUG(
      "REFILLING pd->moved=%ld, oldpd->moved=%ld", (long) pd->moved,
      (long) oldpd->moved);
  ...
  if (avail >= next_chunk_bits) {
    ...
    ASN_DEBUG(
        "!-Parent frame %ld, require %ld, will claim %ld", (long) avail,
        (long) next_chunk_bits, (long) arg->unclaimed);
  }
  pd->buffer = oldpd->buffer;
  pd->nboff  = oldpd->nboff;
  ASN_DEBUG(
      "Refilled pd%s old%s", asn_bit_data_string(pd),
