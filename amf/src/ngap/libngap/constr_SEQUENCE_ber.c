...
asn_dec_rval_t SEQUENCE_decode_ber(
  ...
  switch (ctx->phase) {
    case 0:
      ...
      if (ctx->left >= 0) ctx->left += rval.consumed; /* ?Subtracted below! */
    ...
    case 1:
      ...
      for (edx = ((size_t) ctx->step >> 1); edx < td->elements_count;
        ...
        if (ctx->left < 0 && ((const uint8_t*) ptr)[0] == 0) {
          if (LEFT < 2) {
            ...
            ASN_DEBUG(
                "edx = %" ASN_PRI_SIZE ", opt = %d, ec=%d", edx,
                elements[edx].optional, td->elements_count);
        ...
        if (n == opt_edx_end) {
          ...
          if (!IN_EXTENSION_GROUP(specs, edx + elements[edx].optional)) {
            ASN_DEBUG(
                "Unexpected tag %s (at %" ASN_PRI_SIZE ")",
                ber_tlv_tag_string(tlv_tag), edx);
            ASN_DEBUG(
                "Expected tag %s (%s)%s", ber_tlv_tag_string(elements[edx].tag),
                elements[edx].name,
            ...
            ASN_DEBUG(
                "Skipping unexpected %s (at %" ASN_PRI_SIZE ")",
                ber_tlv_tag_string(tlv_tag), edx);
      ...
      microphase2:
        ...
        if (elements[edx].flags & ATF_OPEN_TYPE) {
          rval = OPEN_TYPE_ber_get(
              opt_codec_ctx, td, st, &elements[edx], ptr, LEFT);
        } else {
        ...
        ASN_DEBUG(
            "In %s SEQUENCE decoded %" ASN_PRI_SIZE
            " %s of %d "
        ...
        switch (rval.code) {
          ...
          case RC_WMORE: /* More data expected */
            ...
            ASN_DEBUG(
                "Size violation (c->l=%ld <= s=%ld)", (long) ctx->left,
                (long) size);
            /* Fall through */
    ...
    case 4: /* only 00's expected */
      ...
      ASN_DEBUG(
          "SEQUENCE %s Leftover: %ld, size = %ld", td->name, (long) ctx->left,
          (long) size);
      ...
      while (ctx->left) {
        ...
        ll = ber_skip_length(
            opt_codec_ctx, BER_TLV_CONSTRUCTED(ptr), (const char*) ptr + tl,
            LEFT - tl);
        switch (ll) {
          case 0:
            if (!SIZE_VIOLATION) RETURN(RC_WMORE);
...
asn_enc_rval_t SEQUENCE_encode_der(
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    erval = elm->type->op->der_encoder(
        elm->type, *memb_ptr2, elm->tag_mode, elm->tag, 0, 0);
    if (erval.encoded == -1) return erval;
    computed_size += erval.encoded;
    ASN_DEBUG(
        "Member %" ASN_PRI_SIZE " %s estimated %ld bytes", edx, elm->name,
        (long) erval.encoded);
  }
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    ASN_DEBUG(
        "Member %" ASN_PRI_SIZE " %s of SEQUENCE %s encoded in %ld bytes", edx,
