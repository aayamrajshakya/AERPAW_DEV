...
ssize_t uper_get_length(
    asn_per_data_t* pd, int ebits, size_t lower_bound, int* repeat) {
  ssize_t value;
...
int uper_get_constrained_whole_number(
    asn_per_data_t* pd, uintmax_t* out_value, int nbits) {
...
int uper_put_constrained_whole_number_u(
    asn_per_outp_t* po, uintmax_t v, int nbits) {
  if (nbits <= 31) {
    return per_put_few_bits(po, v, nbits);
  } else {
    /* Put higher portion first, followed by lower 31-bit */
    if (uper_put_constrained_whole_number_u(po, v >> 31, nbits - 31)) return -1;
...
int per_imax_range_rebase(
    intmax_t v, intmax_t lb, intmax_t ub, uintmax_t* output) {
  uintmax_t range;
...
int per_long_range_rebase(
    long v, intmax_t lb, intmax_t ub, unsigned long* output) {
...
int per_imax_range_unrebase(
    uintmax_t inp, intmax_t lb, intmax_t ub, intmax_t* outp) {
  uintmax_t range;
...
int per_long_range_unrebase(
    unsigned long inp, intmax_t lb, intmax_t ub, long* outp) {
  intmax_t tmp = *outp;
