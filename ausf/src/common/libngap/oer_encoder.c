...
asn_enc_rval_t oer_encode_primitive(
  ...
  ASN_DEBUG(
      "Encoding %s (%" ASN_PRI_SIZE " bytes)", td ? td->name : "", st->size);
...
ssize_t oer_open_type_put(
  ...
  er = td->op->oer_encoder(
      td, constraints, sptr, oer__count_bytes, &serialized_byte_count);
