...
bool mime_parser::parse(const std::string& str) {
  std::string CRLF = "\r\n";
  Logger::ausf_server().debug("Parsing the message with Simple Parser");
  ...
  Logger::ausf_server().debug("Boundary: %s", boundary_str.c_str());
  ...
  while (boundary_pos < boundary_last_post) {
    ...
    if (boundary_pos > 0) {
      ...
      Logger::ausf_server().debug("Body: %s", p.body.c_str());
...
unsigned char* mime_parser::format_string_as_hex(const std::string& str) {
  ...
  Logger::ausf_server().debug(
      ...
      str.c_str());
  Logger::ausf_server().debug("Data (formatted):");
